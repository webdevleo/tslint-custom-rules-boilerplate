...
const rule = 'jsx-icons-name';
const fileName = 'Components/Icons/ComponentName.tsx';
const wrongFilePath = 'Components/Something/index.tsx';
...
describe('jsx-component-name test', () => {
    ...
    it(`testing success functional components with FC type, generic type and null body with passed options`,
        () => {
            const src = 'const ExampleNothing: FC<IProps> = () => null;';
            const result = helper({
                src, rule: {
                    name: rule,
                    options: ['Example']
    ...
    it(`testing success functional components with FC type, generic type and null body`, () => {
        const src = 'const ExampleComp: FC<IProps> = () => null;';
        const result = helper({
            src, rule: {
                name: rule,
                options: ['Example']
    ...
    it(`testing success JSX function with correct prefix name`, () => {
        const src = 'const IconComponent = ({ id, className }) => (<div id={id} className={className}/>);';
        const result = helper({src, rule, fileName});
        expect(result.errorCount).toBe(0);
    });
    ...
    it(`testing success`, () => {
        const src = `
			export class IconComponent extends React.Component implements IComponent {
				render() {
					return null;
				}
			};
    ...
    it(`testing failure functional component not in Icon folder`, () => {
        const src = `
			export Test = () => <div/>;
		`;

        const result = helper({src, rule, fileName: wrongFilePath});
        expect(result.errorCount).toBe(0);
    });
});
